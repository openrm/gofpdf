...
type ImageInfoType struct {
	r     io.Reader     // Raw image data
	buf   *bytes.Buffer // Read buffer
	smask *bytes.Buffer // Soft Mask, an 8bit per-pixel transparency mask
...
func (info *ImageInfoType) load() error {
	info.buf = new(bytes.Buffer)
	if _, err := io.Copy(info.buf, info.r); err != nil {
		return err
	}
	if info.flush != nil {
	...
	info.i = fmt.Sprintf("%x", info.hash.Sum(nil))
	return nil
}

// GobEncode encodes the receiving image to a byte slice.
func (info *ImageInfoType) GobEncode() (buf []byte, err error) {
	data, err := ioutil.ReadAll(info.r)
	if err != nil {
		return
	}
	info.r = bytes.NewReader(data)
	var maskData []byte
	if info.smask != nil {
		maskData = info.smask.Bytes()
	}
	info.r = bytes.NewReader(data)
	fields := []interface{}{data, maskData, info.n, info.w, info.h, info.cs,
...
func (info *ImageInfoType) GobDecode(buf []byte) (err error) {
	var data, maskData []byte
	fields := []interface{}{&data, &maskData, &info.n, &info.w, &info.h,
	...
	info.r = io.TeeReader(dr, hash)
	if len(maskData) > 0 {
		info.smask = bytes.NewBuffer(maskData)
	}
	info.hash = hash
	return info.load()
}
...
func (info *ImageInfoType) addCloseHook(cbs ...func() error) {
	for _, cb := range cbs {
		prev := info.Close
		if prev != nil {
